...
import { spawn, exec, type SpawnOptionsWithoutStdio } from "node:child_process"
...
describe("git", () => {
    test("create, push to, and clone a repo", async () => {
        ...
        await wrapCallback((callback: () => void) => {
            spawn("git", ["push", "http://localhost:" + port + "/xyz/doom", "main"], { cwd: srcDir }).on(
                "exit",
                (code) => {
                    expect(code).toBe(0)
                    callback()
    ...
    test("create, push to, and clone a repo successful", async () => {
        ...
        await wrapCallback((callback: () => void) => {
            spawn("git", ["commit", "-am", "a!!"], { cwd: srcDir }).on("exit", (code) => {
                expect(code).toBe(0)
                callback()
            })
        })
        await wrapCallback((callback: () => void) => {
            spawn("git", ["push", "http://localhost:" + port + "/doom", "main"], {
                cwd: srcDir,
            }).on("exit", (code) => {
                expect(code).toBe(0)
                callback()
    ...
    test("clone into programatic directories", async () => {
        ...
        await wrapCallback((callback: () => void) => {
            spawn("git", ["push", "http://localhost:" + port + "/doom.git", "main"], {
                cwd: srcDir,
            }).on("exit", (code) => {
                expect(code).toBe(0)
                callback()
    ...
    test("test tagging", async () => {
        ...
        await wrapCallback((callback: () => void) => {
            spawn("git", ["push", "--tags", "http://localhost:" + port + "/doom", "main"], { cwd: srcDir }).on(
                "exit",
                (code) => {
                    expect(code).toBe(0)
                    callback()
    ...
    describe("repos list", () => {
        // A working dir containing a single bare `test.git` repo, created at runtime
        // (rather than committing a bare git repo as a fixture).
        const workingRepoDir = `/tmp/${Math.floor(Math.random() * (1 << 30)).toString(16)}`
        // A path that intentionally does not exist, to exercise the error branch.
        const notWorkingRepoDir = `/tmp/${Math.floor(Math.random() * (1 << 30)).toString(16)}`

        beforeAll(async () => {
            fs.mkdirSync(workingRepoDir, { recursive: true })
            await new Promise<void>((resolve) => {
                new Git(workingRepoDir).create("test", () => resolve())
            })
        })
    ...
    test("create, push to, and clone a repo reject", async () => {
        ...
        function _spawn(
            ...
            ps.on("error", (err) => {
                console.error(err.message + " while executing: " + cmd + " " + args?.join(" "))
            })
            return ps
        }
        ...
        await wrapCallback((callback: () => void) => {
            _spawn("git", ["push", "http://localhost:" + port + "/doom", "main"], {
                cwd: srcDir,
            }).on("exit", (code) => {
        ...
        await wrapCallback((callback: () => void) => {
            ...
            // `close` fires after stdio is fully flushed, so `data` is complete and
            // both assertions run before the test proceeds (avoids an exit-vs-stderr race).
            glog.on("close", (code) => {
                expect(code).toBe(128)
                const res =
                    /fatal: bad default revision 'HEAD'/.test(data) || /does not have any commits yet/.test(data)
    ...
    test("should be able to protect certain routes", async () => {
        ...
        await wrapCallback((callback: () => void) => {
            // wrong password -> server responds 401 -> git fails (terminal prompt disabled)
    ...
    test("should be able to send custom messages to git client (main stream)", async () => {
        ...
        await wrapCallback((callback: () => void) => {
            const logs: any[] = []
            const push = spawn("git", ["push", "http://localhost:" + port + "/doom.git", "main"], { cwd: srcDir })
